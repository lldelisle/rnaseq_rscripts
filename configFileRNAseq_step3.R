...
RNAseqFunctionPath<-"~/Dropbox/scripts/toShare/RNAseqNewVersion/RNAseqFunctions_v07.R"
samplesPlan<-"~/Desktop/testRNAscripts/samplesplan_Marian.txt" #This file should be a tabulated file with at least one column called "sample". Optionnaly, the paths to the counts tables and FPKM tables can be provided under the column called: htseq_count_file and cufflinks_file.


#### STEP 3 - PLOTS ###
#Required
tableWithNormalizedExpression<-"~/Desktop/testRNAscripts/AllCufflinks_Simplified_Marian_norm.txt" #You can put here either the FPKM norm values or the count norm values obtained after DESeq2
...
#Optional
outputFolder<-"~/Desktop/testRNAscripts/plots/"
usePng<-F#By default pdf is used as output. If set to T, png will be used.
#fixedColors<-list(tissue=c(ECT="#FC7C76",MES="#17B7FF"),stage=c(E9.5="#E3A208",E10.5="#D87FFF",E11.5="#1CD570",E12.5="#86C306"),Replicate=c(`1`="mediumturquoise",`2`="orchid")) #You can provide color for each value of each factor in you samples plan to have constistant graphs for PCA, correlation and genes.
...
PCA1D<-list(fill="tissue", alpha="stage", linetype="Replicate") 
...
PCA2D<-list(fill="tissue",shape="stage",color="Replicate")
#Possible personalizations are :
...
### Genes ###
fileWithGenes<-"~/Dropbox/scripts/toShare/RNAseqNewVersion/Hoxgenes.txt" #One gene per line. The first line of the gene file should correspond to a column in the expression file.
...
plotGenesPara<-list(color="stage",shape="Batch")
#Possible personalizations are :
#color is for the color of the symbol, 
#alpha is for the transparency,
#shape is for the shape of the symbol.
