...
checkFunctionPath <- function() {
  if (!exists("RNAseqFunctionPath")) {
    ...
    if (!file.exists(RNAseqFunctionPath)) {
      stop("The file provided in RNAseqFunctionPath:", RNAseqFunctionPath,
        " does not exists.")
    }
  }
}
...
if (exists("mergeCounts")) {
  if (!is.logical(mergeCounts)) {
    ...
    if (mergeCounts) {
      ...
      write.table(htseqCounts, file = paste0(outputFolderForStep1, "/AllHTSeqCounts.txt"),
    ...
    if (subsetCounts) {
      ...
      if (exists("htseqCounts")) {
        ...
        if (exists("initialTableWithCount")) {
            ...
            if (!"Ens_ID" %in% colnames(htseqCounts)) {
              if (exists("geneIDColInInitialTable")) {
                if (geneIDColInInitialTable %in% colnames(htseqCounts)) {
                  ...
                  cat("Will subset the counts provided in the initialTableWithCount :",
                  initialTableWithCount, ".\n")
                  name <- basename(file_path_sans_ext(initialTableWithCount))
                  inputOK <- T
                } else {
                  cat("The table provided as initialTableWithCount : ", initialTableWithCount,
                  " does not contain a column called Ens_ID and the geneIDColInInitialTable specified is not part of the column names. The counts will not be subset.\n")
                }
              } else {
              cat("The table provided as initialTableWithCount : ", initialTableWithCount,
                " does not contain a column called Ens_ID and the geneIDColInInitialTable (the column name with the ensembl ids in this table) has not been specified. The counts will not be subset.\n")
              }
            } else {
              cat("Will subset the counts provided in the initialTableWithCount :",
              initialTableWithCount, ".\n")
          ...
          cat("The file provided in the initialTableWithCount :", initialTableWithCount,
            " does not exists. The counts will not be subset.\n")
          }
        } else {
      ...
      if (inputOK) {
        if (exists("genesToRmFromCounts")) {
          if (file.exists(genesToRmFromCounts)) {
            ...
            write.table(htseqCounts, file = paste0(outputFolderForStep1,
              "/", name, "_subset.txt"), sep = "\t", row.names = F, quote = F)
            cat(paste0("The counts have been subset in ", outputFolderForStep1,
              "/", name, "_subset.txt.\n"))
          } else {
            cat("The file specified as genesToRmFromCounts:", genesToRmFromCounts,
              " does not exists. No subset will be done.\n")
          }
        } else {
...
if (exists("mergeFPKM")) {
  if (!is.logical(mergeFPKM)) {
    ...
    if (mergeFPKM) {
      ...
      write.table(FPKMCuff, file = paste0(outputFolderForStep1, "/", name,
  ...
  if (!is.logical(normFPKMWithAnoukMethod)) {
    ...
    if (normFPKMWithAnoukMethod) {
        ...
        if (exists("initialTableWithFPKM")) {
          if (file.exists(initialTableWithFPKM)) {
            ...
            if (length(grep("FPKM_", colnames(FPKMCuff))) == 0) {
              ...
              cat("Will normalize the FPKM provided in the initialTableWithFPKM :",
              initialTableWithFPKM, ".\n")
              name <- basename(file_path_sans_ext(initialTableWithFPKM))
              inputOK <- T
            }
          } else {
          cat("The file provided in the initialTableWithFPKM :", initialTableWithFPKM,
            " does not exists. No normalization will be performed.\n")
          }
        } else {
      ...
      if (inputOK) {
        ...
        write.table(resl[["normData"]], file = paste0(outputFolderForStep1,
          "/", name, "_norm.txt"), sep = "\t", row.names = F, quote = F)
        cat(paste0("The FPKM have been normalized in ", outputFolderForStep1,
          "/", name, "_norm.txt.\n"))
        write.table(data.frame(sample = names(resl$normcoeff), coef = resl$normcoeff),
          file = paste0(outputFolderForStep1, "/", name, "_coeffsUsedForNorm.txt"),
          sep = "\t", row.names = F, quote = F)
        cat(paste0("The coefficients used to normalize are in ", outputFolderForStep1,
          "/", name, "_coeffsUsedForNorm.txt.\n"))
        if (exists("keepGenesUsedForNorm")) {
          if (is.logical(keepGenesUsedForNorm)) {
            if (keepGenesUsedForNorm) {
              write.table(data.frame(Ens_ID = resl[["hkgenesENSID"]], gene_name = resl[["hkgenesName"]]),
              file = paste0(outputFolderForStep1, "/", name, "_HKGenesUsedForNorm.txt"),
              sep = "\t", row.names = F, quote = F)
              cat(paste0("The genes used to normalize are written in ", outputFolderForStep1,
              "/", name, "_HKGenesUsedForNorm.txt.\n"))
            }
          }
        }
      }
